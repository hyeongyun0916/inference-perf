...
def test_forward_directive_protects_new_increment(monkeypatch):
    ...
    -> [1,2,3], so LCP depth = 3 (where inherited history ends). The new
    prompt tail is [3, 5); the output is protected via a covers_output flag
    (server-resolved, no overcover)."""
    ...
    # Ranged directive for the new prompt tail [depth, prompt_len) — exact —
    # plus a covers_output flag for the output (server resolves the exact
    # post-prompt region; no client-side overcover).
    assert dirs is not None and len(dirs) == 2
    assert dirs[0]["start"] == 3 and dirs[0]["end"] == 5
    ...
    assert dirs[1]["priority"] == dirs[0]["priority"]
    assert dirs[0]["duration"] > 0 and dirs[1]["duration"] == dirs[0]["duration"]
