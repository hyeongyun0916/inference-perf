...
def test_covers_output_passthrough(monkeypatch):
    monkeypatch.setattr(m, "_render_token_ids", _fake_render(TOKMAP))
    # Flag segments need no coordinate calibration: the policy emits a
    # range-less covers_output directive and the SERVER resolves the exact
    # post-prompt region it generates. Calibration passes them through.
    profile = [
        ReuseSegment(start=0, end=999, breadth=3, cold_gap=1, end_msg=1),
        ReuseSegment(start=999, end=999, breadth=1, cold_gap=9,
                     covers_output=True),
    ]
    o, payload = _obj(profile, MSGS, max_tokens=100)
    out = asyncio.run(o._calibrate_profile_via_render(payload))
    assert (out[0].start, out[0].end) == (0, 30)      # ranged: calibrated
    assert out[1].covers_output is True
    assert (out[1].start, out[1].end) == (999, 999)   # flag: untouched
